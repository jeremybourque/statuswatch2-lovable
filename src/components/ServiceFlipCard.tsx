        ...
        <div
          ...
          <div className="w-full flex-1 flex flex-col" onMouseLeave={() => setHoveredDay(null)}>
              ...
              {backView === 'graph' && (() => {
                ...
                return (
                  <svg viewBox={`0 0 ${vbW} ${vbH}`} className="w-full flex-1" preserveAspectRatio="none">
                    {/* Y-axis labels */}
                    {yTicks.map((t, i) => {
                    ...
                    {/* X-axis day labels */}
                    {dayLabels.map((d, i) => (
                      <text key={i} x={d.x} y={oY + chartH + 9} textAnchor="middle" className="fill-muted-foreground" style={{ fontSize: '10px' }}>
                        {d.label}
                      </text>
                    ))}
                    {/* Grid lines */}
